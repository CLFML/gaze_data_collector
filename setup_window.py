...
# from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, 
#                             QPushButton, QComboBox, QFormLayout, QMessageBox,
#                             QGroupBox, QSpinBox, QDialog, QApplication)
# from PyQt5.QtCore import Qt, QTimer
# from PyQt5.QtGui import QPixmap, QImage
# import cv2
# import numpy as np
...
#     def __init__(self, metadata_path=None, parent=None):
#         super().__init__(parent)
...
#         # Setup UI and initialize camera preview
#         self.setup_ui()
#         self.setup_camera()
    
#     def setup_ui(self):
#         """Initialize the setup UI components."""
...
#             # TODO: Add MediaPipe face detection here to verify subject position
...
#     def get_current_setup_folder(self):
#         """Generate the folder name for the current setup."""
#         yaw = self.yaw_angles[self.yaw_combo.currentIndex()]
#         pitch = self.pitch_angles[self.pitch_combo.currentIndex()]
#         distance = self.distances[self.distance_combo.currentIndex()]
        
#         return f"Y{yaw:02d}_P{pitch:02d}_D{distance}"
    
#     def start_experiment(self):
#         """Start the experiment with current setup."""
#         if not self.start_btn.isEnabled():
#             return
            
#         try:
#             if self.metadata_path:
#                 # Normal operation with metadata
#                 base_dir = os.path.dirname(os.path.dirname(self.metadata_path))
#             else:
#                 # Test mode - create temporary directory
#                 base_dir = "test_experiment"
                
#             setup_folder = os.path.join(base_dir, "AngleSetups", 
#                                       self.get_current_setup_folder())
...
#             with open(os.path.join(setup_folder, "setup_config.json"), 'w') as f:
#                 json.dump(setup_config, f, indent=2)
            
#             # TODO: Launch experiment window
#             # self.experiment_window = ExperimentWindow(setup_folder, setup_config)
#             # self.experiment_window.show()
#             # self.close()
            
#             QMessageBox.information(self, "Success", 
#                                   f"Setup configuration saved to {setup_folder}")
...
#     # Create test metadata path
#     test_metadata_path = None  # For testing without metadata
    
#     # Create and show the setup window
#     window = SetupWindow(metadata_path=test_metadata_path)
#     window.show()
